...
export class MinPmtMcpServer {
  ...
  private registerTools() {
    ...
    this.server.tool('create-ticket', 'Create a ticket with optional metadata', CreateSchema.shape, {
      title: 'Create Ticket',
      description: 'Create a new ticket in the current project with title, status, priority, labels, assignee, and due date.',
    ...
    // move-ticket
    const MoveSchema = z.object({ ticketId: z.string(), newStatus: z.enum(['todo', 'in-progress', 'done']) });
    this.server.tool('move-ticket', 'Move ticket to a new status', MoveSchema.shape, {
      title: 'Move Ticket',
    ...
    // list-tickets
    const ListSchema = z.object({ status: z.enum(['todo', 'in-progress', 'done']).optional(), priority: z.enum(['low', 'medium', 'high', 'critical']).optional() });
    this.server.tool('list-tickets', 'List tickets with optional filters', ListSchema.shape, {
      title: 'List Tickets',
    ...
    this.server.tool('get-ticket', 'Get a ticket by ID', GetSchema.shape, {
      title: 'Get Ticket',
    ...
    this.server.tool('update-ticket', 'Update ticket fields', UpdateSchema.shape, {
      title: 'Update Ticket',
      description: 'Update one or more fields on a ticket by ID. Fields can include title, description, status, priority, labels, assignee, due.',
      idempotentHint: true,
      destructiveHint: false,
    }, async (args) => {
    ...
    this.server.tool('delete-ticket', 'Delete a ticket by ID', DeleteSchema.shape, {
      title: 'Delete Ticket',
